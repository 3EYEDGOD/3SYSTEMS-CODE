...
SWQCPERSON=$( cat bios-tmp/CC-Person.txt)
...
echo "ORDER INFORMATION" >> bios-tmp/LINE-Output.txt

echo "Order Number: $ORDER" >> bios-tmp/LINE-Output.txt
...
if $(cat bios-tmp/$SERIAL-DIR.txt | cut -d '"' -f1 | sed "s,/$,," | fgrep -wqi -e "Debug"); then
...
cat bios-tmp/$SERIAL-IPMI-SUM.txt| egrep -i "IPv4 Address           : "|cut -d ":" -f2 > bios-tmp/$SERIAL-IPMI-IP-ADDR.txt
IPMIIP=$(cat bios-tmp/$SERIAL-IPMI-IP-ADDR.txt | xargs)
...
cat bios-tmp/$SERIAL-STD-Parts.txt | grep -i "IPMI Password"| cut -d "|" -f2-3 | tr -d "|"  > bios-tmp/$SERIAL-IPMI-Password.txt
...
cat bios-tmp/$SERIAL-BIOS-Default.txt | grep -i loaded
BIOSSTATUS=$(cat bios-tmp/$SERIAL-BIOS-Default.txt)

tr -s ' ' < bios-tmp/$SERIAL-BIOS-Default.txt | grep -i loaded > bios-tmp/$SERIAL-BIOS-Reset.txt
BIOSDEFAULT=$(cat bios-tmp/$SERIAL-BIOS-Reset.txt)


# Verify BIOS changed

if $(echo "$BIOSDEFAULT" | grep -oh "\w*"loaded"\w*"| fgrep -wqi -e "loaded"); then
