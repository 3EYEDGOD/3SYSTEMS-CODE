...
if $(cat /tmp/ix-tmp/$SERIAL-dir.txt | cut -d '"' -f1 | sed "s,/$,," | fgrep -wqi -e "Debug"); then
...
lynx --dump /tmp/ix-tmp/$SERIAL-PBS-Passmark_Log.cert.htm | fgrep "TEST RUN" > /tmp/ix-tmp/$SERIAL-test-run.txt
...
lynx --dump /tmp/ix-tmp/$SERIAL-PBS-Passmark_Log.cert.htm | egrep -i 'CPU 0|CPU 1' > /tmp/ix-tmp/$SERIAL-CPU-presence.txt
...
lynx --dump /tmp/ix-tmp/$SERIAL-PBS-Passmark_Log.cert.htm | egrep -i 'CPU 0|CPU 1' > /tmp/ix-tmp/$SERIAL-CPU-temp.txt
...
lynx --dump /tmp/ix-tmp/$SERIAL-PBS-Passmark_Log.cert.htm | fgrep "Disk (00)" > /tmp/ix-tmp/$SERIAL-disk00-pf.txt
DISK00PF=$(cat /tmp/ix-tmp/$SERIAL-disk00-pf.txt)

# Collects test duration

lynx --dump /tmp/ix-tmp/$SERIAL-PBS-Passmark_Log.cert.htm | fgrep "Test Duration" > /tmp/ix-tmp/$SERIAL-testduration-pf.txt
...
cat /tmp/ix-tmp/$SERIAL-ipmi-sum.txt | egrep -i "IPv4 Address           : " | cut -d ":" -f2 > /tmp/ix-tmp/$SERIAL-ipmi-ipadddress.txt
...
cat /tmp/ix-tmp/$SERIAL-ipmi-sum.txt | egrep -i "BMC MAC Address        : " > /tmp/ix-tmp/$SERIAL-ipmi-bmc-mac.txt
...
lynx --dump /tmp/ix-tmp/$SERIAL-PBS-Passmark_Log.cert.htm | fgrep "Motherboard manufacturer" > /tmp/ix-tmp/$SERIAL-motherboard-manufacturer.txt
...
lynx --dump /tmp/ix-tmp/$SERIAL-PBS-Passmark_Log.htm | fgrep "System Model:" > /tmp/ix-tmp/$SERIAL-system-model.txt
...
dmidecode -t processor | egrep -i 'cpu|manufacturer|serial|size|speed|core|version|manufactuer' >> /tmp/ix-tmp/system-output.txt
dmidecode -t processor | egrep -i 'cpu|manufacturer|serial|size|speed|core|version|manufactuer' > /tmp/ix-tmp/processor.txt

for cpus in `dmidecode -t4 | awk '/Handle / {print $2}'`; do
   echo `dmidecode -t4 | sed '/Flags/,/Version/d' | egrep -A20 "Handle ${cpus}" | grep -m 1 "Socket Designation" | grep -o '.\{0,0\}:.\{0,18\}' | tr -d '\:| '`; >> /tmp/ix-tmp/system-output.txt
   echo `dmidecode -t4 | sed '/Flags/,/Version/d' | egrep -A20 "Handle ${cpus}" | grep -m 1 "Family" | grep -o '.\{0,0\}:.\{0,18\}' | tr -d '\:| '`; >> /tmp/ix-tmp/system-output.txt
   echo `dmidecode -t4 | sed '/Flags/,/Version/d' | egrep -A20 "Handle ${cpus}" | grep -m 1 "Manufacturer" | grep -o '.\{0,0\}:.\{0,18\}' | tr -d '\:| '`; >> /tmp/ix-tmp/system-output.txt
   echo `dmidecode -t4 | sed '/Flags/,/Version/d' | egrep -A20 "Handle ${cpus}" | grep -m 1 "Current Speed" | grep -o '.\{0,0\}:.\{0,18\}' | tr -d '\:| '`; >> /tmp/ix-tmp/system-output.txt
   echo `dmidecode -t4 | sed '/Flags/,/Version/d' | egrep -A20 "Handle ${cpus}" | grep -m 1 "Voltage" | grep -o '.\{0,0\}:.\{0,18\}' | tr -d '\:| '`; >> /tmp/ix-tmp/system-output.txt
   echo `dmidecode -t4 | sed '/Flags/,/Version/d' | egrep -A20 "Handle ${cpus}" | grep -m 1 "Core Count" | grep -o '.\{0,0\}:.\{0,18\}' | tr -d '\:| '`; >> /tmp/ix-tmp/system-output.txt
done
...
dmesg | egrep -i 'Ethernet|Network' >> /tmp/ix-tmp/system-output.txt
dmesg | egrep -i 'Ethernet|Network' > /tmp/ix-tmp/nicinfo.txt
...
dmidecode -t memory | egrep -i 'manufacturer|serial|size|speed|locator' | grep -i hynix | wc -l >> /tmp/ix-tmp/system-output.txt
dmidecode -t memory | egrep -i 'manufacturer|serial|size|speed|locator' | grep -i hynix | wc -l > /tmp/ix-tmp/mem.txt

MEMINFO=$(cat  /tmp/ix-tmp/mem.txt)

echo "POPULATION, TYPE, SPEED, AND SIZE:" >> /tmp/ix-tmp/system-output.txt

dmidecode -t memory | egrep "(^\s+Locator:|Speed|Manufacturer:|^\s+Volatile Size:)" | cut -d: -f2 | pr -at5 -s, >> /tmp/ix-tmp/system-output.txt
dmidecode -t memory | egrep "(^\s+Locator:|Speed|Manufacturer:|^\s+Volatile Size:)" | cut -d: -f2 | pr -at5 -s, >> /tmp/ix-tmp/volatile-memsize.txt
...
ipmitool sel list | egrep -c "Voltage #0x54" >> /tmp/ix-tmp/system-output.txt
ipmitool sel list | egrep -c "Voltage #0x54" >> /tmp/ix-tmp/voltage-selerrors.txt
