...
lynx --dump ix-tmp/"$SERIAL"-Passmark_Log.html | grep -F "TEST RUN" > ix-tmp/"$SERIAL"-test-run.txt
tr -s ' ' <  ix-tmp/"$SERIAL"-test-run.txt | cut -d ' ' -f 4 > ix-tmp/"$SERIAL"-pf.txt

PASSFAIL=$(cat ix-tmp/"$SERIAL"-pf.txt | xargs)

if $(echo $PASSFAIL | grep -oh "\w*PASSED\w*" | grep -F -wqi -e PASSED); then
...
lynx --dump ix-tmp/"$SERIAL"-Passmark_Log.html | grep -E -i 'CPU 0|CPU 1' > ix-tmp/"$SERIAL"-CPU-presence.txt
...
lynx --dump ix-tmp/"$SERIAL"-Passmark_Log.html | grep -E -i 'CPU 0|CPU 1' > ix-tmp/"$SERIAL"-CPU-temp.txt
...
lynx --dump ix-tmp/"$SERIAL"-Passmark_Log.html | grep -F "Disk (00)" > ix-tmp/"$SERIAL"-disk00-pf.txt
DISK00PF=$(cat ix-tmp/"$SERIAL"-disk00-pf.txt | xargs)

# Collects test duration

lynx --dump ix-tmp/"$SERIAL"-Passmark_Log.html | grep -F "Test Duration" > ix-tmp/"$SERIAL"-testduration-pf.txt
...
cat ix-tmp/"$SERIAL"-ipmi_lan.txt | grep -E -i "IP Address              : " | cut -d ":" -f2 > ix-tmp/"$SERIAL"-ipmi-ipadddress.txt
IPMIIP=$(cat ix-tmp/"$SERIAL"-ipmi-ipadddress.txt | xargs)

# Collect IPMI IP address

cat ix-tmp/"$SERIAL"-ipmi_lan.txt | grep -E -i "MAC Address             : " | xargs | cut -d ' ' -f 4 > ix-tmp/"$SERIAL"-ipmi-bmc-mac.txt
...
lynx --dump ix-tmp/"$SERIAL"-Passmark_Log.html | grep -F "Motherboard Manufacturer:" | xargs | cut -d ' ' -f3 > ix-tmp/"$SERIAL"-motherboard-manufacturer.txt
MOTHERMAN=$(cat ix-tmp/"$SERIAL"-motherboard-manufacturer.txt)

# Getting system model type

lynx --dump ix-tmp/"$SERIAL"-Passmark_Log.html | grep -F "Motherboard Model:" | xargs | cut -d " " -f3 > ix-tmp/"$SERIAL"-system-model.txt
...
if $(echo "$MOTHERMAN" | grep -F -wqi -e GIGABYTE); then
...
if $(echo "$MOTHERMAN" | grep -F -wqi -e GIGABYTE) && $(echo "$PWC" | grep -oh "\w*Complete\w*" | grep -F -wqi -e Complete); then
...
cat ix-tmp/"$SERIAL"-ifconfig.txt | grep -E -A5 -i --color '(o1:|o2:)' | xargs -0 | sed 's/^ *//g' >> ix-tmp/mac-address-list.txt
echo -e "\n==========================================================================" >> ix-tmp/mac-address-list.txt
...
if $(echo "$MODELTYPE" | grep -F -wqi -e IX-4224GP2-IXN); then
